...
struct kmem_cache *quantumqueue;
	...
	size_t count, loff_t *fpos)
{
	...
	{
		mdata->data[quantumnum] = kmem_cache_alloc(quantumqueue, GFP_KERNEL);
		if(mdata->data[quantumnum] == NULL)
		{
			goto out;
		}
		memset(mdata->data[quantumnum], 0, dev->quantum);
	}
...
static int __init scull_init(void)
{
	...
	dev_t scull_dev;
	#if(SCULL_MAJOR == 0)
	ret = alloc_chrdev_region(&scull_dev, 0, scull_nr_dev, "scullc");
	#else
	scull_dev = MKDEV(scullmajor, scullminor);
	ret = register_chrdev_region(scull_dev, scull_nr_dev, "scullc");
	#endif
	if(ret < 0)
	{
		goto out;
	...
	myclass = class_create(THIS_MODULE, "scullcclass");
	if(myclass == NULL)
	{
		goto out3;
	}
	...
	for(i = 0; i < scull_nr_dev; i++)
	{
		...
		sprintf(buffer, "scullc%d", i);
		device_create(myclass, NULL, scull_dev, NULL, buffer);
	}

	quantumqueue = kmem_cache_create("scullc", scull_quantum, 0, SLAB_HWCACHE_ALIGN, NULL);
	if(quantumqueue == NULL)
	{
		goto out4;
	}
...
static void scull_exit(void)
{
	...
	kmem_cache_destroy(quantumqueue);
	kfree(scull_devices);
